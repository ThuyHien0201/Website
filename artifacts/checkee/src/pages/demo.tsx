...
const emptyStep = (name = "", desc = ""): ProcessStep => ({
  name,
  date: "",
  materials: "",
  desc,
...
export default function Demo() {
  const { user, openLoginModal, openPricingModal, trackJourney, activePlan } =
    useAuth();
  ...
  const handleBizCertUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.forEach((file) => {
      ...
      reader.onload = (ev) => {
        setBizCerts((prev) => [
          ...prev,
          { certName: file.name, preview: ev.target?.result as string },
        ]);
      };
  ...
  const handleNext = () => {
    if (currentStep === 1) {
      trackJourney(
        "completed_step1",
        "Hoàn tất bước 1 - Thông tin doanh nghiệp",
        1,
        { roles: roles.join(","), company: companyName, phone: companyPhone },
      );
    } else if (currentStep === 2) {
      trackJourney(
        "completed_step2",
        "Hoàn tất bước 2 - Thông tin sản phẩm",
        2,
      );
    } else if (currentStep === 3) {
      trackJourney(
        "completed_step3",
        "Hoàn tất bước 3 - Hành trình sản phẩm",
        3,
      );
      if (!user) {
  ...
  const handleDownloadOrPrint = (action: "download" | "print") => {
    trackJourney(
      action === "download" ? "clicked_download_qr" : "clicked_print_qr",
      `Bấm ${action === "download" ? "Tải" : "In"} mã QR`,
      4,
    );
    if (!activePlan) {
    ...
    <div className="min-h-[100dvh] bg-white flex flex-col font-sans">
      <section className="py-14 bg-gradient-to-b from-[#EFF5FF] to-white border-b border-[#E5EAF0]">
        <div className="container max-w-[1280px] mx-auto px-6 lg:px-8 text-center">
          <h1 className="text-3xl lg:text-5xl font-bold text-[#1557B0] mb-4">
            Tự tay tạo trang truy xuất chỉ trong 2 phút
          </h1>
          <p className="text-[#4A5868] text-lg max-w-2xl mx-auto">
            Nhập thông tin doanh nghiệp, sản phẩm và xem mã QR được tạo tự động.
            Hoàn toàn miễn phí.
          </p>
          {user && !activePlan && (
            <div className="inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-full text-sm font-semibold bg-[#fef3e2] text-[#ed8302]">
              <QrCodeIcon className="w-4 h-4" /> Bạn đang dùng bản thử —{" "}
              <button
                ...
                <div className="flex items-center justify-between mb-10 relative">
                  ...
                  {steps.map((step) => {
                    ...
                    return (
                      <div
                        key={step.id}
                        className="flex flex-col items-center gap-2 bg-white px-2 relative"
                      >
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-colors ${active ? "bg-[#1557B0] text-white" : completed ? "bg-[#0B2D8A] text-white" : "bg-[#E5EAF0] text-[#7D9E94]"}`}
                        >
                          {completed ? (
                        ...
                        <div
                          className={`text-xs font-semibold ${active || completed ? "text-[#1557B0]" : "text-[#7D9E94]"}`}
                        >
                          {step.label}
                        </div>
                      </div>
                      ...
                      <div className="space-y-5">
                        <div>
                          ...
                          <p className="text-sm text-[#7D9E94] mt-0.5">
                            Cho chúng tôi biết về tổ chức của bạn
                          </p>
                        </div>

                        <div className="grid sm:grid-cols-2 gap-4">
                          <div className="sm:col-span-2 space-y-1.5">
                            <Label className="text-[#1557B0] font-semibold flex items-center gap-1.5">
                              <Building2 className="w-4 h-4" /> Tên doanh nghiệp{" "}
                              <span className="text-red-500">*</span>
                            </Label>
                            <Input
                              value={companyName}
                              onChange={(e) => setCompanyName(e.target.value)}
                              placeholder="VD: Công ty TNHH Nông sản Xanh"
                              className="h-10 rounded-xl border-[#E5EAF0]"
                            />
                          </div>
                          <div className="sm:col-span-2 space-y-1.5">
                            <Label className="text-[#1557B0] font-semibold flex items-center gap-1.5">
                        ...
                        <div className="space-y-2">
                          <Label className="text-[#1557B0] font-semibold">
                            Vai trò của bạn{" "}
                            <span className="text-xs text-[#7D9E94] font-normal">
                              (chọn được nhiều)
                            </span>
                          </Label>
                          <div className="grid grid-cols-3 gap-3">
                            {(
                              ["manufacturer", "farm", "distributor"] as Role[]
                            ).map((role) => (
                              <button
                                key={role}
                                type="button"
                              ...
                              >
                                ...
                                {roles.includes(role) && (
                                  <CheckCircle2 className="w-4 h-4 text-[#1557B0]" />
                                )}
                              </button>
                            ))}
                          </div>
                        ...
                        <div className="space-y-2">
                          <Label className="text-[#1557B0] font-semibold">
                            Upload chứng chỉ / giấy phép doanh nghiệp
                          </Label>
                          <input
                            ref={bizCertRef}
                            type="file"
                          ...
                          {bizCerts.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                              {bizCerts.map((c, i) => (
                                <div
                                  key={i}
                                  className="flex items-center gap-2 bg-[#EFF5FF] text-[#1557B0] px-3 py-1.5 rounded-full text-xs font-semibold"
                                >
                    ...
                    {currentStep === 2 && (
                      <div className="space-y-5">
                        <div>
                          ...
                          <p className="text-sm text-[#7D9E94] mt-0.5">
                            Điền thông tin để hiển thị trên trang truy xuất
                          </p>
                        </div>
                        <div className="grid sm:grid-cols-2 gap-4">
                          <div className="sm:col-span-2 space-y-1.5">
                            ...
                            <Input
                              ...
                              placeholder="VD: Cà phê Robusta Đà Lạt"
                            />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-[#1557B0] font-semibold">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <Input
                              ...
                              placeholder="VD: Hộp 12 gói x 20g"
                            />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-[#1557B0] font-semibold">
                            ...
                            <select
                              className="flex h-9 w-full rounded-md border border-[#E5EAF0] bg-transparent px-3 py-1 text-sm text-[#0F1B2D]"
                              value={unit}
                          ...
                          <div className="sm:col-span-2 space-y-1.5">
                            <Label className="text-[#1557B0] font-semibold">
                              Tên chứng nhận sản phẩm
                            </Label>
                            <Input
                              value={certName}
                              onChange={(e) => setCertName(e.target.value)}
                              placeholder="VD: VietGAP, GlobalGAP, OCOP 4 sao..."
                            />
                          </div>
                          <div className="sm:col-span-2 space-y-1.5">
                            <Label className="text-[#1557B0] font-semibold">
                        ...
                        <div className="space-y-1.5">
                          ...
                          >
                            ...
                            {productImageName && (
                              <span className="relative z-10 bg-white/90 text-[#1557B0] text-xs font-medium px-2 py-0.5 rounded-full">
                                {productImageName}
                              </span>
                            )}
                          </button>
                      ...
                      <div className="space-y-5">
                        <div>
                          <h3 className="text-lg font-bold text-[#1557B0]">
                            Hành trình sản phẩm
                            {roles.length > 0 && (
                              <span className="ml-2 text-sm font-normal text-[#7D9E94]">
                          ...
                          <p className="text-sm text-[#7D9E94] mt-0.5">
                            {roles.length === 0
                              ? "Bạn chưa chọn vai trò ở bước 1 — quay lại để chọn."
                              : step3Label}
                          </p>
                        </div>
                          ...
                          <div className="space-y-3">
                            <div className="space-y-3">
                              {processSteps.map((step, i) => (
                                <div
                                  key={i}
                                  className="p-4 border border-[#E5EAF0] rounded-xl bg-[#FAFBFC] relative group"
                                >
                                  <button
                                    onClick={() =>
                                      setProcessSteps((prev) =>
                                  ...
                                  <div className="flex items-center gap-2 mb-3">
                                    <div className="w-6 h-6 rounded-full bg-[#1557B0] text-white text-xs font-bold flex items-center justify-center shrink-0">
                                      {i + 1}
                                    </div>
                                    <span className="text-xs font-bold text-[#1557B0] uppercase tracking-wide">
                                      Bước {i + 1}
                                    </span>
                                  </div>
                                  <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-1">
                                      <Label className="text-xs text-[#4A5868]">
                                        Tên bước
                                      </Label>
                                      <Input
                                        value={step.name}
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <Input
                                        ...
                                        value={step.date}
                                        onChange={(e) =>
                                          updateStep(i, "date", e.target.value)
                                        }
                                        className="h-9"
                                      />
                                    </div>
                                    <div className="space-y-1">
                                      ...
                                      <Input
                                        ...
                                        placeholder="VD: Phân bón, thuốc BVTV..."
                                        className="h-9"
                                      />
                                    </div>
                                    <div className="space-y-1">
                                      ...
                                      <Input
                                        value={step.desc}
                                        onChange={(e) =>
                                          updateStep(i, "desc", e.target.value)
                                        }
                                        className="h-9"
                                      />
                                    </div>
                          ...
                          <div className="space-y-4">
                            <div className="border border-[#E5EAF0] rounded-xl overflow-hidden">
                              <div className="flex items-center gap-2 px-4 py-3 bg-[#EFF5FF]">
                                <div className="w-7 h-7 bg-[#1557B0] rounded-lg flex items-center justify-center">
                                  <span className="text-white text-xs font-bold">
                                    NCC
                                  </span>
                                </div>
                                <span className="font-semibold text-[#1557B0] text-sm">
                                  Nhà cung cấp
                                </span>
                              </div>
                              <div className="px-4 pb-4 pt-3 grid sm:grid-cols-2 gap-3">
                                ...
                                <div className="sm:col-span-2 space-y-1.5">
                                  ...
                                  <Input
                                    ...
                                    placeholder="VD: 456 Lê Lợi, Q.1, TP.HCM"
                                    className="h-9"
                                  />
                                </div>
                                <div className="space-y-1.5">
                            ...
                            <div className="border border-[#E5EAF0] rounded-xl overflow-hidden">
                              <div className="flex items-center gap-2 px-4 py-3 bg-[#fef3e2]">
                                <div className="w-7 h-7 bg-[#ed8302] rounded-lg flex items-center justify-center">
                                  <span className="text-white text-xs font-bold">
                                    NM
                                  </span>
                                </div>
                                <span className="font-semibold text-[#ed8302] text-sm">
                                  Người mua
                                </span>
                              </div>
                              <div className="px-4 pb-4 pt-3 grid sm:grid-cols-2 gap-3">
                                ...
                                <div className="sm:col-span-2 space-y-1.5">
                                  ...
                                  <Input
                                    ...
                                    placeholder="VD: 789 Trần Hưng Đạo, Q.5"
                                    className="h-9"
                                  />
                                </div>
                                <div className="space-y-1.5">
                        ...
                        {roles.length === 0 && (
                          <div className="bg-[#fef3e2] border border-[#fdba74]/40 rounded-xl p-4 flex items-start gap-3">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-[#ed8302]">
                                Chưa chọn vai trò
                              </p>
                              <p className="text-xs text-[#4A5868] mt-0.5">
                                Quay lại bước 1 để chọn ít nhất một vai trò.
                              </p>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="bg-[#EFF5FF] border border-[#DCEEFF] rounded-xl p-4 flex items-start gap-3">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-[#1557B0]">
                                Đăng ký để tạo mã QR
                              </p>
                              <p className="text-xs text-[#4A5868] mt-0.5">
                                Nhấn "Tạo mã QR" bên dưới để đăng ký miễn phí và
                                nhận 1 lượt tạo QR ngay.
                              </p>
                            </div>
                          </div>
                        )}
                    ...
                    {currentStep === 4 && (
                      <div className="space-y-6">
                        <div className="text-center space-y-2">
                          ...
                          <h3 className="text-xl font-bold text-[#1557B0]">
                            Mã QR đã sẵn sàng!
                          </h3>
                          <p className="text-sm text-[#4A5868]">
                            Quét mã QR bên phải để xem trang truy xuất. In hoặc
                            tải về để gắn lên sản phẩm.
                          </p>
                        </div>
                        ...
                        {!activePlan && (
                          <div className="bg-gradient-to-r from-[#EFF5FF] to-[#DCEEFF] border border-[#DCEEFF] rounded-xl p-4">
                            <div className="flex items-center justify-between gap-4">
                              <div>
                                <p className="text-sm font-bold text-[#1557B0]">
                                  Nâng cấp để in nhiều mã QR
                                </p>
                                <p className="text-xs text-[#4A5868] mt-0.5">
                                  Bản thử chỉ cho in 1 mã. Gói trả phí: in không
                                  giới hạn, quản lý nhiều SKU.
                                </p>
                              </div>
                              <Button
                                onClick={() => openPricingModal("download_qr")}
                                className="shrink-0 bg-[#1557B0] hover:bg-[#0D3F8A] text-white rounded-full text-xs px-4 h-9"
                              >
                                Xem gói
                              </Button>
                            </div>
                        ...
                        {showPrintPricingHint && !activePlan && (
                          <div className="bg-[#fef3e2] border border-[#fdba74]/40 rounded-xl p-4 text-center">
                            <p className="text-sm text-[#ed8302] font-semibold">
                              Chọn gói để in nhiều mã, hoặc bỏ qua để in 1 mã
                              miễn phí.
                            </p>
                            <button
                              onClick={doDownload}
                              className="text-xs text-[#4A5868] underline mt-2 hover:text-[#0F1B2D]"
                            >
                              Tải 1 mã QR ngay
                            </button>
                          </div>
                        ...
                        <div className="border-t border-[#E5EAF0] pt-4 text-center">
                          <p className="text-xs text-[#7D9E94]">
                            Muốn quản lý nhiều sản phẩm và phân quyền nhân viên?
                          </p>
                          <button
                            onClick={() => openPricingModal()}
                            className="text-xs text-[#1557B0] font-semibold underline underline-offset-2 mt-1"
                          >
                  ...
                  <div className="flex justify-between items-center mt-10 pt-6 border-t border-[#E5EAF0]">
                    {currentStep > 1 ? (
                      <Button
                        ...
                        className="text-[#4A5868] border-[#E5EAF0]"
                      >
                        Quay lại
                      </Button>
                    ) : (
            ...
            <div className="lg:col-span-5 lg:sticky lg:top-24 h-fit">
                  ...
                  <div className="flex-1 pb-10">
                    <div className="px-4 py-3 border-b border-[#E5EAF0] flex items-center justify-between bg-white sticky top-0 z-20">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-[#1557B0]">
                          Checkee
                        </span>
                      </div>
                    ...
                    <div className="px-4 py-4 bg-white">
                      <h2 className="text-lg font-bold text-[#0F1B2D] mb-0.5">
                        {productName || "Tên sản phẩm"}
                      </h2>
                      {companyName && (
                        <p className="text-[10px] text-[#7D9E94]">
                          {companyName}
                        </p>
                      )}
                      <div className="flex items-center gap-2 flex-wrap mt-1">
                        {gtin && (
                          <span className="text-[10px] text-[#7D9E94] font-mono">
                            GTIN: {gtin}
                          </span>
                        )}
                        {batch && (
                          <span className="text-[10px] text-[#7D9E94]">
                            Lô: {batch}
                          </span>
                        )}
                      </div>
                      <div className="mt-2 flex gap-1.5 flex-wrap">
                        {roles.map((r) => (
                          <span
                            key={r}
                            className="inline-block bg-[#EFF5FF] text-[#1557B0] px-2 py-0.5 rounded-full text-[10px] font-semibold"
                          >
                            {ROLE_LABELS[r]}
                          </span>
                        ))}
                        {weight && unit && (
                          <span className="inline-block bg-[#F4F6F8] text-[#4A5868] px-2 py-0.5 rounded-full text-[10px] font-semibold">
                            {weight} {unit}
                          </span>
                        )}
                        {certName && (
                          <span className="inline-block bg-[#fef3e2] text-[#ed8302] px-2 py-0.5 rounded-full text-[10px] font-semibold">
                            {certName}
                          </span>
                        )}
                      </div>
                    ...
                    {(productionDate || productExpiry || spec) && (
                      <div className="px-4 py-3 bg-[#FAFBFC] border-y border-[#E5EAF0]">
                        <div className="grid grid-cols-2 gap-2">
                          {productionDate && (
                            <div>
                              <div className="text-[9px] text-[#7D9E94] uppercase mb-0.5">
                                Ngày SX
                              </div>
                              <div className="text-xs text-[#0F1B2D] font-semibold">
                                {productionDate}
                              </div>
                            </div>
                          )}
                    ...
                    {companyAddress && (
                      <div className="px-4 py-3 bg-white border-b border-[#E5EAF0]">
                        <div className="uppercase text-[9px] text-[#7D9E94] font-bold mb-1">
                          ĐỊA CHỈ DN
                        </div>
                        <div className="text-xs text-[#0F1B2D]">
                          {companyAddress}
                        </div>
                      </div>
                    )}
                    ...
                    {showsProcess && processSteps.some((s) => s.name) && (
                      <div className="px-4 py-3 bg-[#FAFBFC] border-b border-[#E5EAF0]">
                        <div className="uppercase text-[9px] text-[#7D9E94] font-bold mb-2">
                          {step3Label.toUpperCase()}
                        </div>
                        <div className="space-y-2">
                          {processSteps
                            .filter((s) => s.name)
                            .map((step, i) => (
                              <div key={i} className="flex gap-2">
                                <div className="w-4 h-4 rounded-full bg-[#1557B0] text-white text-[8px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                                  {i + 1}
                                </div>
                                <div>
                                  ...
                                  {step.date && (
                                    <div className="text-[9px] text-[#7D9E94]">
                                      {step.date}
                                    </div>
                                  )}
                                  {step.desc && (
                                    <div className="text-[9px] text-[#4A5868]">
                                      {step.desc}
                                    </div>
                                  )}
                                </div>
                    ...
                    {showsDistributor && (supplierName || buyerName) && (
                      <div className="px-4 py-3 bg-[#FAFBFC] border-b border-[#E5EAF0]">
                        {supplierName && (
                          <div className="mb-2">
                            <div className="text-[9px] text-[#7D9E94] uppercase font-bold mb-1">
                              NHÀ CUNG CẤP
                            </div>
                            <div className="text-xs font-semibold text-[#0F1B2D]">
                              {supplierName}
                            </div>
                            {supplierAddress && (
                              <div className="text-[9px] text-[#7D9E94]">
                    ...
                    {qrReady && (
                      <div
                        className="px-4 py-6 flex flex-col items-center gap-4"
                        ref={qrRef}
                      >
                        <QRCodeCanvas
                          value={previewUrl}
                          ...
                          includeMargin
                        />
                        <p className="text-[10px] text-[#7D9E94] text-center">
              ...
              {!activePlan && (
                <div className="mt-6 w-[300px] mx-auto bg-gradient-to-r from-[#EFF5FF] to-[#DCEEFF] border border-[#DCEEFF] rounded-2xl p-4 text-center">
                  <p className="text-sm font-bold text-[#1557B0] mb-1">
                    Quản lý nhiều sản phẩm?
                  </p>
                  <p className="text-xs text-[#4A5868] mb-3">
                    Nâng cấp để tạo QR không giới hạn, quản lý SKU và phân quyền
                    nhân viên.
                  </p>
                  <button
                    onClick={() => openPricingModal()}
