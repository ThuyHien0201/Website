...
export default function Dashboard() {
  ...
  const certStatusBadge = (status: string) => {
    if (status === "valid") return (
      <span className="inline-flex items-center gap-1.5 bg-[#D4EDE6] text-[#1A6B52] text-xs font-semibold px-3 py-1 rounded-full">
            ...
            {sideItems.map(item => (
              ...
              >
                ...
                {item.id === "certs" && userCerts.filter(c => c.status === "valid").length > 0 && (
                  <span className="ml-auto bg-[#1A6B52] text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
