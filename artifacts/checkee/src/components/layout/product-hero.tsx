  ...
  return (
    <section className="relative pt-28 pb-16 lg:pt-36 lg:pb-20 overflow-hidden bg-[#FAFBFC]">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(#1557B0 1px, transparent 1px), linear-gradient(90deg, #1557B0 1px, transparent 1px)",
          backgroundSize: "48px 48px",
        }}
      />
      <div className="absolute -top-32 -right-32 w-[480px] h-[480px] rounded-full bg-[#EFF5FF] blur-3xl opacity-60 pointer-events-none" />
      ...
      <div className="container max-w-[1280px] mx-auto px-6 lg:px-8 relative">
        ...
        <nav className="flex items-center gap-2 text-xs text-[#7D9E94] mb-6">
          <Link href="/" className="hover:text-[#1557B0] transition-colors">
            Trang chủ
          </Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#7D9E94]">Giải pháp</span>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#1557B0] font-semibold">{eyebrow}</span>
        </nav>

        <div className="grid lg:grid-cols-12 gap-10 lg:gap-12 items-center">
          {/* Left content */}
          <div className="lg:col-span-6 space-y-6 relative z-10">
            <div className="inline-flex items-center gap-2 bg-white border border-[#E5EAF0] text-[#1557B0] px-3 py-1.5 rounded-full text-[11px] font-semibold uppercase tracking-wider shadow-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-[#ed8302]"></span>
              {eyebrow}
            </div>
            <h1 className="text-3xl lg:text-5xl font-bold text-[#1557B0] leading-[1.15]">
              {title}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              ...
              <Link href={secondaryCta.href}>
                <Button
                  variant="outline"
                  className="rounded-full border-2 border-[#1557B0] text-[#1557B0] hover:bg-[#1557B0] hover:text-white px-6 py-3 h-auto font-semibold"
                >
                  {secondaryCta.label}
                </Button>
              </Link>
            ...
            {stats && stats.length > 0 && (
              <div className="grid grid-cols-3 gap-6 pt-6 border-t border-[#E5EAF0]">
                {stats.map((s, i) => (
                  <div key={i}>
                    <div className="text-2xl lg:text-3xl font-bold text-[#1557B0]">
                      {s.value}
                    </div>
          ...
          <div className="lg:col-span-6 relative">
            <div className="relative">
              ...
              <div className="absolute -top-4 -left-4 w-24 h-24 lg:w-32 lg:h-32 bg-[#1557B0] rounded-3xl -z-10" />
              ...
              {badge && (
                <div className="absolute -bottom-6 left-6 lg:left-12 bg-white p-4 rounded-2xl shadow-xl border border-[#E5EAF0] flex items-center gap-3 max-w-[260px]">
                  <div className="bg-[#EFF5FF] text-[#1557B0] p-2.5 rounded-xl shrink-0">
                    <badge.icon className="w-5 h-5" />
                  </div>
                  <div className="text-sm font-semibold text-[#1557B0] leading-snug">
                    {badge.label}
                  </div>
                </div>
              )}
